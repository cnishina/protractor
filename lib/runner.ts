...
export class Runner extends EventEmitter {
   ...
   *     are finished.
   */
  async runTestPreparer(extraFlags?: string[]): Promise<any> {
    ...
    await this.plugins_.onPrepare();
    await helper.runFilenameOrFn_(this.config_.configDir, this.preparer_);
  }
  ...
  afterEach(): Promise<void> {
    ...
    if (this.config_.restartBrowserBetweenTests) {
      this.restartPromise = this.restartPromise || protractor.browser.restart();
  ...
  async createBrowser(plugins: any, parentBrowser?: ProtractorBrowser): Promise<ProtractorBrowser> {
    ...
    browser_.ready = Promise.resolve(browser_);
    ...
    browser_.restart = async(): Promise<any> => {
      await this.driverprovider_.quitDriver(browser_.driver);
      let newBrowser = await replaceBrowser();
      newBrowser.ready = Promise.resolve(newBrowser);
    };
    return browser_;
  }
  ...
  async run(): Promise<number> {
    ...
    if (this.config_.restartBrowserBetweenTests) {
      ...
      let restartDriver = () => {
        if (!this.frameworkUsesAfterEach) {
          this.restartPromise = browser_.restart();
        }
      };
    ...
    results = await require(frameworkPath).run(this, this.config_.specs);
